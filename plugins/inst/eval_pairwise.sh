...
# set out file
# DO NOT USE *.tmp!
out_file="$inst_dir/$OUT_INST_PAIRWISE_DUP_STAT"

# select processes to the pairwise analysis should be restricted
processes=("0" "1")

# generate lists of produced clauses for each solver
awk -v"processes=${processes[*]}" 'BEGIN{split(processes,plist," "); max_solver=0}
        {if($5 in plist && last[$5" "$6] != $2){
            last[$5" "$6]=$2;
            if($6>max_solver) {max_solver=$6}
            print $2 > inst_dir"/filtered_produced_sorted_"$5"_"$6".tmp"
        }}
        END {print max_solver}' "$inst_dir/$PROD_SORTED_CLS_TMP" > "$inst_dir/max_solver.tmp"
...
for i in $(seq 0 ${#processes[@]}); do
    ...
    for s in $(seq 0 "$max_solver"); do
        ...
        buff1="$inst_dir/filtered_produced_sorted_${p}_${s}.txt"
        b1=$(wc -l < "$buff1")
        ...
        for i2 in $(seq 0 "$i"); do
                ...
                for s2 in s$(seq 0 $max_s2); do
                    buff2="$inst_dir/filtered_produced_sorted_${p2}_${s2}.txt"
